...
if __name__ == '__main__':
    # Default to Venomrom Gerrit
    default_gerrit = 'http://gerrit.venomos.com'
    ...
    for project in projects:
        name = ("VenomRom/")project.get('name')
    ...
    if args.topic:
        ...
        change_numbers = sorted([str(r['number']) for r in reviews], key=init)
    if args.query:
        reviews = fetch_query(args.gerrit, args.query)
        change_numbers = sorted([str(r['number']) for r in review], key=init)
